...
class Profiler():
    ...
    def rpn_parallel(self, rpn_head, conv, feature, i):
        ...
        top_n_idx_ = model.rpn._get_top_n_idx(box_cls_, [num_anchors_per_level[0]])
        ...
        # objectness_prob.append(torch.sigmoid(box_cls_))
        ...
        # return torch.sigmoid(box_cls_), level_[batch_idx, top_n_idx_], proposal_[batch_idx, top_n_idx_]

    def rpn_neo(self):
        # # load features (debug mode)
        ...
        for feature in self.args["features_"]:
            ...
            self.rpn_parallel(rpn_head, conv, feature, i)
            
            # profiling
            def _rpn_parallel():
                self.rpn_parallel(rpn_head, conv, feature, i)
            self._profile_helper(_rpn_parallel, f"rpn_parallel_f{i}")
            # dependencies
            shape_, size_ = _size_helper(feature)
            if i != 4:
                dependencies.write(f"layer_{i}, rpn_parallel_f{i}, {shape_}, {size_}\n")
            else:
                dependencies.write(f"extra, rpn_parallel_f{i}, {shape_}, {size_}\n")
            add_ = 0
            shape_, size_ = _size_helper(box_cls_)
            add_ += float(size_.rstrip(" Mb"))
            shape_, size_ = _size_helper(level_)
            add_ += float(size_.rstrip(" Mb"))
            shape_, size_ = _size_helper(proposal_)
            add_ += float(size_.rstrip(" Mb"))
            add_ = f"{round(add_, sigfigs=4)} Mb"
            dependencies.write(f"rpn_parallel_f{i}, rpn_merger, 3 Tensors, {add_}\n")

            i += 1

        # post processing step (merging and computing)
        ...
        def _rpn_merger():
            ...
            for boxes, scores, lvl, img_shape in zip(proposals, objectness_prob, levels, [(800, 800)]):  # run only once since batch=1
                ...
                final_boxes.append(boxes)
                final_scores.append(scores)
        self._profile_helper(_rpn_merger, "rpn_merger")
    ...
    def roi_box_roi_pool(self):
        ...
        dependencies.write(f"rpn_merger, box_roi_pool, {shape_}, {size_}\n")
    ...
    def postprocess_detection(self):
        ...
        shape_, size_ = _size_helper(self.args["proposals"])
        dependencies.write(f"rpn_merger, postprocess_detections, {shape_}, {size_}\n")
